...
from explicit_ortho_conv import ECO
...
class LipBlock(nn.Module):
    def __init__(
        self, in_planes, planes, conv_layer, activation_name, stride=1, kernel_size=3
    ):
...
class LipConvNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(
            init_channels, block_size, SOC, activation, stride=2, kernel_size=3
        )
        self.layer2 = self._make_layer(
            self.in_planes, block_size, SOC, activation, stride=2, kernel_size=3
        )
        self.layer3 = self._make_layer(
            self.in_planes, block_size, ECO, activation, stride=2, kernel_size=3
        )
        self.layer4 = self._make_layer(
            self.in_planes, block_size, ECO, activation, stride=2, kernel_size=3
        )
        self.layer5 = self._make_layer(
            self.in_planes, block_size, ECO, activation, stride=2, kernel_size=1
        )
    ...
    def _make_layer(
        self, planes, num_blocks, conv_layer, activation, stride, kernel_size
    ):
        ...
        for stride, kernel_size in zip(strides, kernel_sizes):
            layers.append(
                LipBlock(
                    self.in_planes, planes, conv_layer, activation, stride, kernel_size
                )
            )
